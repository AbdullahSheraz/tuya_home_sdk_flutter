...
class TuyaHomeSdkFlutter {
  ...
  Future<void> initSdk(
    ...
    assert(
      appKey.trim().isNotEmpty && !appKey.contains(" "),
      "AppKey can't be empty or contain whitespaces",
    );
    assert(
      appSecret.trim().isNotEmpty && !appSecret.contains(" "),
      "AppSecret can't be empty or contain whitespaces",
    );

    final res = await methodChannel.invokeMethod<bool>('initSDK', {
      'appKey': appKey.trim(),
    ...
    if (res == true) {
      ...
      debugPrint("Failed to initialize Tuya SDK");
    }
  }
  ...
  Future<bool> checkCodeWithUserName({
    ...
    try {
      var res = await methodChannel
          .invokeMethod<bool>('checkCodeWithUserName', {
            'username': username.trim(),
            'country_code': countryCode.trim(),
            'code': code.trim(),
            'type': type,
  ...
  Future<bool> loginWithPhonePassword({
    ...
    try {
      var res = await methodChannel
          .invokeMethod<bool>('loginWithPhonePassword', {
            'username': phone.trim(),
            'country_code': countryCode.trim(),
            'password': password,
          });
  ...
  Future<ThingSmartUserModel?> updateUserIcon({String? icon}) async {
    try {
      var res = await methodChannel.invokeMethod('updateUserIcon', {
        'iconData': icon,
      });
      if (res) {
        return getUserInfo();
  ...
  /// }
  ///
  Future<ThingSmartUserModel?> updateUserNickName({String? nickname}) async {
    if (nickname == null) {
      return null;
    }
    try {
  ...
  Future<bool> assignDeviceToRoom({
    ...
    try {
      var res = await methodChannel.invokeMethod('assignDeviceToRoom', {
        'deviceId': deviceId,
        'homeId': homeId,
        'roomId': roomId,
      });
  ...
  Future<bool> removeHome({required num homeId}) async {
    try {
  ...
  Future<List<ThingSmartHomeModel>> getHomeList() async {
    try {
      ...
      if (res == null) return [];
      return List<ThingSmartHomeModel>.from(
        res.map((e) => ThingSmartHomeModel.fromJson(e.cast<String, dynamic>())),
      );
    } on PlatformException catch (e) {
      _log(e);
      return [];
  ...
  /// Throws:
  /// - None.
  Future<List<ThingSmartRoomModel>> getHomeRooms({required num homeId}) async {
    try {
      var res = await methodChannel.invokeMethod<List<dynamic>>(
        'getHomeRooms',
        {"homeId": homeId},
      );
      if (res == null) return [];
      return List<ThingSmartRoomModel>.from(
        res.map((e) {
  ...
  Future<ThingSmartDeviceModel?> startConfigWiFiDevice({
    ...
    try {
      var res = await methodChannel.invokeMethod<dynamic>(
        'startConfigWiFiDevice',
        {
          "ssid": ssid,
          "password": password,
          "token": token,
  ...
  Future<ThingSmartDeviceModel?> startConfigSubDevice({
    ...
    try {
      var res = await methodChannel.invokeMethod<dynamic>(
        'startConfigSubDevice',
        {"devId": devId, "timeout": timeout},
      );
      _log(res);
      return ThingSmartDeviceModel.fromJson(res.cast<String, dynamic>());
    } on PlatformException catch (e) {
  ...
  Future<String?> requestHomeToken({required num homeId}) async {
    try {
  ...
  Future<List<ThingSmartDeviceModel>> getHomeDevices({
    ...
    try {
      ...
      var res = await methodChannel.invokeListMethod<dynamic>(
        'getHomeDevices',
        {"homeId": homeId},
      );
      if (res == null) return [];
  ...
  /// Throws:
  /// - None.
  Stream<dynamic> onDeviceDpsUpdated({required String deviceId}) {
    methodChannel.invokeMethod("listenForDevice", {"deviceId": deviceId});
  ...
  //   assert(homeId > 0, "homeId must be greater than 0");
  //   try {
  //     var res = await methodChannel.invokeListMethod<Map<Object?, Object?>>(
  //       'getSceneList',
  //       {"homeId": homeId},
  //     );
  ...
  Future<String?> addHomeMember({
    ...
    try {
      final res = await methodChannel.invokeMethod('addMember', {
        "homeId": homeId,
        "name": name,
  ...
  Future<List<ThingSmartHomeMemberModel>> queryMemberList({
    ...
    try {
      final res = await methodChannel.invokeListMethod('queryMemberList', {
        "homeId": homeId,
      });
      if (res == null) return [];
      return List<ThingSmartHomeMemberModel>.from(
        res.map(
          (e) => ThingSmartHomeMemberModel.fromJson(e.cast<String, dynamic>()),
        ),
      );
    } catch (e) {
      _log(e);
  ...
  Future<bool> removeMember({required num memberId}) async {
    try {
  ...
  Future<bool> joinHome({required String invitationCode}) async {
  ...
  Future<bool> cancelInvitation({required int invitationCode}) async {
  ...
  Future<List<ThingSmartInvitationModel>> queryInvitationList({
    ...
    assert(homeId > 0);
    try {
      var res = await methodChannel.invokeListMethod('queryInvitations', {
        "homeId": homeId,
      });
      if (res == null) return [];
      return List<ThingSmartInvitationModel>.from(
        res.map(
          (e) => ThingSmartInvitationModel.fromJson(e.cast<String, dynamic>()),
        ),
      );
    } on PlatformException catch (e) {
      _log(e);
