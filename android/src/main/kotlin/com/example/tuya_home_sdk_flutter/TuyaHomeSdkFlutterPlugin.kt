...
class TuyaHomeSdkFlutterPlugin : FlutterPlugin, MethodCallHandler,
    ...
    try {
        ...
        // Initialize the SDK
        ThingHomeSdk.init(mContext as Application, appKey!!, appSecret!!)

        result.success(true)
    } catch (e: Exception) {
        result.error("SDK_INIT_ERROR", e.localizedMessage, null)
    }
}
...
//        device["homeId"] = deviceModel.hoem
    ...
    ThingHomeSdk.getBleOperator().startLeScan(scanSetting) { bean: ScanDeviceBean ->
        println("Discovered device productId=${bean.productId}")
    ...
    result.success(null) // Let Flutter know scan started
    }
